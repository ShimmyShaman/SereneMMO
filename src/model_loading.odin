...
GLTFNode :: struct {
  ...
  vertices, indices: rawptr,
  
  vb: VertexBufferResourceHandle,
  ib: IndexBufferResourceHandle,
  mb: BufferResourceHandle,

  albedo_index: int,
  // frag_ubo: ResourceHandle,

  // chunks: [16 * 16]TerrainChunk,
  // index_buffer: IndexBufferResourceHandle,
  // vertex_ary: [CHUNK_DIM_SIZE * CHUNK_DIM_SIZE]TerrainVertex,

  children: [dynamic]GLTFNode,
}
...
load_model :: proc(vctx: ^vi.VkSDLContext, file_name: string) -> (asset: ^GLTFAsset, prs: ProcResult) {
  ...
  // fmt.println("data.animations:", data.animations)
  ...
  asset = new(GLTFAsset)
  asset.nodes = make([]^GLTFNode, len(data.nodes))
  for node, i in data.nodes {
    asset.nodes[i] = create_node(vctx, data, asset, node) or_return
  }
